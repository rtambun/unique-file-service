...
class VideoServiceTest {
    ...
    public void getThumbnailForVideo() throws IOException, FileServiceException {
        InputStream inputStream = new FileInputStream(new ClassPathResource("sample-no-rotate.mp4").getFile());
        ...
        ByteArrayInputStream thumbNailStream = (ByteArrayInputStream) videoService
                .getThumbnail("incidentId", "sample-no-rotate.mp4");
        ...
        assertThat(thumbNail.length).isEqualTo(5350);

        FileInputStream fis = new FileInputStream(new ClassPathResource("sample-thumb-no-rotate.jpg").getFile());
        ...
        verify(mockFileService, times(1))
                .getFileAsInputStream("incidentId", "sample-no-rotate.mp4");

        assertThat(Duration.between(before, after).get(ChronoUnit.SECONDS)).isLessThan(1);
    }

    @Test
    public void getThumbnailForVideo_rotate90() throws FileServiceException, IOException {
        InputStream inputStream = new FileInputStream(new ClassPathResource("sample-rotate.mp4").getFile());
        when(mockFileService.getFileAsInputStream(any(), any())).thenReturn(inputStream);
        when(mockApplicationProperties.getConfigValue(Constants.DEFAULT_THUMBNAIL_WIDTH_KEY)).thenReturn("200");
        when(mockApplicationProperties.getConfigValue(Constants.DEFAULT_THUMBNAIL_HEIGHT_KEY)).thenReturn("200");
        ...
        Instant before = Instant.now();
        ByteArrayInputStream thumbNailStream = (ByteArrayInputStream) videoService
                .getThumbnail("incidentId", "sample-rotate.mp4");
        Instant after = Instant.now();
        log.info("Time to execute get thumbnail for video {}" , Duration.between(before, after));
        ...
        verify(mockFileService, times(1))
                .getFileAsInputStream("incidentId", "sample-rotate.mp4");

        assertThat(Duration.between(before, after).get(ChronoUnit.SECONDS)).isLessThan(1);
    }

    @Test
    public void getThumbnailForVideo_empty() throws FileServiceException, IOException {
        InputStream inputStream = new FileInputStream(new ClassPathResource("empty.mp4").getFile());
        when(mockFileService.getFileAsInputStream(any(), any())).thenReturn(inputStream);
        when(mockApplicationProperties.getConfigValue(Constants.DEFAULT_THUMBNAIL_WIDTH_KEY)).thenReturn("200");
        when(mockApplicationProperties.getConfigValue(Constants.DEFAULT_THUMBNAIL_HEIGHT_KEY)).thenReturn("200");

        FileServiceException exception = assertThrows(FileServiceException.class,
                () -> videoService.getThumbnail("incidentId", "empty.mp4"));

        verify(mockFileService, times(1))
                .getFileAsInputStream("incidentId", "empty.mp4");
    }

    @Test
    public void getThumbnailForVideo_IOExceptionThrow_CheckFileServiceException() throws IOException, FileServiceException {
        InputStream inputStream = new FileInputStream(new ClassPathResource("sample-no-rotate.mp4").getFile());
        inputStream.close();
        when(mockFileService.getFileAsInputStream(any(), any())).thenReturn(inputStream);
        when(mockApplicationProperties.getConfigValue(Constants.DEFAULT_THUMBNAIL_WIDTH_KEY)).thenReturn("200");
        ...
        FileServiceException exception = assertThrows(FileServiceException.class,
                () -> videoService.getThumbnail("incidentId", "sample-no-rotate.mp4"));
